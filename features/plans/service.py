...
class PlanService(BaseService):
    ...
    def move_plan_order(self, plan_id: str, driver_id: str, vehicle_id: str, order_id: str, direction: int) -> PlanDetail:
    ...
    def generate_plan(self, plan_id: str) -> PlanDetail:
    ...
    def evaluate_plan(self, plan_id: str) -> PlanDetail:
        plan = self.get_plan(plan_id)
        if not plan:
            raise ValueError(f"Plan not found: {plan_id}")
    ...
    def validate_plan(self, plan_id: str) -> PlanValidationResult:
        ...
        for row in rows:
            ...
            if not row["driver_active"]:
                errors.append(f"Inactive driver in {r_str}")
            if not row["vehicle_active"]:
                errors.append(f"Inactive vehicle in {r_str}")
            if row["req_driver_adr"] and not row["skill_adr"]:
                errors.append(f"Missing ADR skill in {r_str}")
            if row["req_driver_ehbo"] and not row["skill_ehbo"]:
                errors.append(f"Missing EHBO skill in {r_str}")
            if row["req_vehicle_liftgate"] and not row["spec_liftgate"]:
                errors.append(f"Missing Liftgate in {r_str}")
...
def evaluate_plan_routes_internal(rows: list[dict], travel_times: dict[tuple[str, str], int], start_hour: int = 9) -> dict:
    evaluated: list[dict] = []
    ...
    for row in rows:
        ...
        current_time = route_clocks.get(route, datetime(2026, 1, 1, start_hour))
        origin_zip = route_locations.get(route, row["start_zip"])
        destination_zip = row["destination_zip"]

        travel_time = travel_times.get((origin_zip, destination_zip))
        if travel_time is None:
            raise ValueError(f"No distance matrix entry from {origin_zip} to {destination_zip}")
        ...
        evaluated.append({
            ...
            "driving_time_min": travel_time,
            "departure_time": departure.strftime("%H:%M"),
            "arrival_time": arrival.strftime("%H:%M"),
        })

        route_clocks[route] = arrival
        route_locations[route] = destination_zip
