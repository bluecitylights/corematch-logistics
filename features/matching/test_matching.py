...
def test_matching_router(tmp_path, monkeypatch):
    db_path = setup_test_db(tmp_path)
    
    # Monkeypatch the DB_PATH used in router
    from features.matching import router as match_router
    monkeypatch.setattr(match_router, "DB_PATH", db_path)
