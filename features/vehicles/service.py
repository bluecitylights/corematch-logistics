...
class VehicleService(BaseService):
    ...
    def list_vehicles(self, include_inactive: bool = False) -> Sequence[Vehicle]:
        ...
        return self.query_models(Vehicle, f"SELECT * FROM vehicles {where} ORDER BY vehicle_index")
    ...
    def create_vehicle(self, payload: VehicleCreate) -> Vehicle:
        ...
        veh = self.query_model_or_none(Vehicle, "SELECT * FROM vehicles WHERE vehicle_id = ?", [payload.vehicle_id])
        if veh is None:
    ...
    def update_vehicle(self, vehicle_id: str, payload: VehicleUpdate) -> Vehicle | None:
        ...
        if not fields:
            return self.query_model_or_none(Vehicle, "SELECT * FROM vehicles WHERE vehicle_id = ?", [vehicle_id])
        self.execute_update("vehicles", "vehicle_id", vehicle_id, fields)
        return self.query_model_or_none(Vehicle, "SELECT * FROM vehicles WHERE vehicle_id = ?", [vehicle_id])
    ...
    def count_active(self) -> int:
        row = self.execute("SELECT COUNT(*) FROM vehicles WHERE is_active").fetchone()
